...
let width width box = { box with width; width_mode = Fixed; min_width = width }

let height height box =
  { box with height; height_mode = Fixed; min_height = height }
      ...
      let state =
        if delta_x <> 0. || delta_y <> 0. then
          ...
          let mouse_x, mouse_y =
            (Vector2.x mouse_position, Vector2.y mouse_position)
          in
          mouse_motion_driver mouse_x mouse_y delta_x delta_y box state
        else state
      in
      let state = check_mouse_button MouseButton.Left Mouse.Left box state in
      let state = check_mouse_button MouseButton.Right Mouse.Right box state in
      let state =
        check_mouse_button MouseButton.Middle Mouse.Middle box state
      in
      let state =
        check_mouse_button MouseButton.Forward Mouse.Forward box state
      in
      let state = check_mouse_button MouseButton.Back Mouse.Back box state in
      let state = check_mouse_button MouseButton.Side Mouse.Side box state in
      let state = check_mouse_button MouseButton.Extra Mouse.Extra box state in
      set_window_min_size
        (max (int_of_float box.min_width) 1)
        (max (int_of_float box.min_height) 1);
      set_window_max_size
        (int_of_float box.max_width)
        (int_of_float box.max_height);
      begin_drawing ();
      draw box;
      end_drawing ();
      loop state
